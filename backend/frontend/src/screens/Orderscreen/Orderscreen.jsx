...
const stripePromise = loadStripe(
  "pk_test_51MbkNeGJ8v9b2yrMsOEfEwwuEkzRpZOrJ2A5Wkdti8WqCdwI7b0BXIFGAwX888Qpd6K8fZG07igiitpOGOEE52Ns00Aj9fGYtL"
);
const Orderscreen = () => {
  ...
  const addToCart = (item) => {
    const existingItemsWithSameId = cartItems.filter(
      (cartItem) =>
        cartItem.id === item.id &&
        stringifyAndSortToppings(cartItem.toppings) ===
          stringifyAndSortToppings(item.toppings)
    );
    const totalQuantityWithSameId = existingItemsWithSameId.reduce(
    ...
    if (totalQuantityWithSameId < item.countInStock) {
      ...
      if (existingItemIndex !== -1) {
        ...
        setCartItems([
          ...cartItems,
          { ...item, uniqueId: generateUniqueId(), quantity: 1 },
        ]);
        setOpenAccordion(null);
      }
    } else {
      setMessage(
        "You have reached the maximum available stock for this pizza."
      );
    }
  };
                      ...
                      <StyledTextContainer>
                        ...
                        <Accordion
                          ...
                          onChange={(e, isExpanded) =>
                            setOpenAccordion(isExpanded ? item.id : null)
                          }
                        ...
                        >
                          ...
                          <StyledToppingsContainer>
                            ...
                            {Object.keys(extraToppings).map(
                              (topping, index) => {
                                const isChecked = (
                                  tempToppings[uniqueId] || []
                                ).includes(topping);
                          ...
                          >
                            <StyledButton
                              disabled={
                                item.countInStock <= 0 || showPaymentForm
                              }
                              onClick={() =>
                                addToCart({
                                  ...item,
                                  toppings: tempToppings[uniqueId] || [],
                                })
                              }
                            >
                              Add to cart
                ...
                <StyledCartContainer>
                  ...
                  {cartItems.map((cartItem, index) => {
                    const menuItem = orderItems.find(
                      (orderItem) => orderItem.id === cartItem.id
                    );
                    return (
                      <StyledCartItem id="cart-checkout" key={cartItem.id}>
                        <StyledButtonContainer>
                          <StyledH2>
                            ...
                            {Array.isArray(cartItem.selectedToppings) &&
                          ...
                          <CartButton
                            ...
                            disabled={
                              cartItem.quantity >= menuItem.countInStock
                            }
                          >
                            <FaPlus />
                          </CartButton>
